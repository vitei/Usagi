...
namespace usg
{
	...
	static const VkImageLayout g_layoutMap[] =
	{
		...
		VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,			// LAYOUT_TRANSFER_SRC_OPTIMAL
		VK_IMAGE_LAYOUT_PRESENT_SRC_KHR				// LAYOUT_PRESENT_SRC_OPTIMAL
	};
	...
	VkAccessFlags GetAccessFlags(uint32 uFlags)
	{
		...
		if (uFlags & RenderPassDecl::AC_SHADER_WRITE_BIT)
		{
			flagsOut |= VK_ACCESS_SHADER_WRITE_BIT;
		}
		if (uFlags & RenderPassDecl::AC_TRANSFER_WRITE_BIT)
		{
...
void RenderPass::Init(GFXDevice* pDevice, const RenderPassInitData &initData, uint32 uId)
{
	...
	for (uint32 i = 0; i < decl.uAttachments; i++)
	{
		...
		case RenderPassDecl::ATTACH_DEPTH:
			attachmentDescriptions[i].format = pDevice->GetPlatform().GetDepthFormat(in.format.eDepth);
			break;
		default:
			ASSERT(false);
		}
