	...
	namespace Systems
	{
		...
		public:
			...
			static void OnEvent(const Inputs& inputs, Outputs& outputs, const usg::SetShadowQuality& evt)
			{
				...
				outputs.sceneComp.GetRuntimeData().shadowQuality.bPointLights = evt.bEnablePointLights;
				outputs.sceneComp.GetRuntimeData().shadowQuality.bSpotLights = evt.bEnableSpotLights;
